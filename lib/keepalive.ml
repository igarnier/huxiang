...
let rec loop state =
  ...
  | Message m, AwaitAck i ->
    (match m with
     ...
     | Transport.Json _ ->
       Lwt.fail_with "huxiang: received data instead of Ack - aborting\n"
    )      
...
and handle_protocol_message message state =
  state.log_callback ("read message: "^(Json.to_string (state.in_to_json message)));
  ...
  (match m_opt with
   | None     -> Lwt.return ()
   | Some msg -> Transport.write_message state.socket (Transport.Json (state.out_to_json msg)));%lwt
  loop state
